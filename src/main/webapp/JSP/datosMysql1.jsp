...
<html lang="es">
    ...
    <body class="bg-green-200 flex items-center justify-center min-h-screen">
        <div class="w-full max-w-4xl p-6 bg-white rounded-lg shadow-lg">
            ...
            <c:choose>
                <c:when test="${consulta == 'numeroalumnociclo'}">
                    <h2 class="text-3xl font-black text-blue-700 mb-6 text-center mb-4 "> Numero de alumnos en cada ciclo</h2>
                </c:when>
                <c:otherwise>
            ...
            <table class="min-w-full bg-white border border-gray-300">                             
                <thead>
                    <tr class="text-xl font-black text-blue-700 bg-blue-400">
                        ...
                        <th class="py-2 px-4 border-b">Numero de Alumnos</th>
                    </tr>
                </thead>
                <tbody>
                    <c:forEach var="alum" items="${alumnList}" varStatus="status">
                        <tr class="${status.index % 2 == 0 ? 'bg-blue-100' : 'bg-white'} text-lg font-bold text-blue-500">
                            <td class="py-2 px-4 border-b text-center">${alum.ciclo}</td>
                            <td class="py-2 px-4 border-b text-center">${alum.numAlumnosMatriculados}</td>
                        </tr>
                    </c:forEach>
                </tbody>
            </table>
