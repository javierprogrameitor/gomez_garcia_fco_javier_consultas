...
<html lang="es">
    ...
    <body class="bg-green-200 flex items-center justify-center min-h-screen">
        <div class="w-full max-w-4xl p-6 bg-white rounded-lg shadow-lg">
            ...
            <c:choose>
                <c:when test="${consulta == 'edadaritmetica'}">
                    <h2 class="text-3xl font-black text-blue-700 mb-6 text-center mb-4 ">Alumnos varones cuya edad sea inferior a la media aritmética en años, que hayan nacido en un año par y en domingo</h2>
                </c:when>
                <c:otherwise>
                ...
                <tbody>
                    <c:forEach var="alum" items="${alumnList}" varStatus="status">
                        <tr class="${status.index % 2 == 0 ? 'bg-blue-100' : 'bg-white'} text-lg font-bold text-blue-500">
                            <td class="py-2 px-4 border-b text-center"><c:out value="${alum.nombre}" /></td>
                            <td class="py-2 px-4 border-b text-center"><c:out value="${alum.apellidos}" /></td>
                            <td class="py-2 px-4 border-b text-center"><c:out value="${alum.edad}" /></td>
                        </tr>
                    </c:forEach>
                </tbody>
            </table>
